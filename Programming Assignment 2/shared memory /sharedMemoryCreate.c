...
//    printf("numberOfSlaves = %d",numberOfSlaves);
...
//    printf("fileName = %s",fileName);
    ...
    for (int processNumber =0; processNumber < numberOfSlaves; processNumber++) {
        /*random number between 1 to 3*/
        randomnumber = (rand() % (2 + 1 - 1)) + 1;
//        printf("%d\n", randomnumber);
//        printf("Calling from processNUmber %d \n",processNumber);
        if((childPid = fork()) < 0) {
            perror("Fork Failure");
            exit(1);
        }
...
//            printf("child process created!!!\n");
            ...
            char *slaveOptions[] = {"./sharedMemoryClient", "-n", nValue,"-i", iValue, "-l",fileName ,(char *)0};
            
//            printf("%d\n", randomnumber);
//            sleep(randomnumber);
            int execCmd = execv("./sharedMemoryClient", slaveOptions);
            
            
//            sleep(randomnumber);
//            printf("execCmd = %d",execCmd);
//            system("./sharedMemoryClient -n 10");
                    }
    }
